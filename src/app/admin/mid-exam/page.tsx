...
import {
  ...
  FaFileExcel, FaPaperPlane, FaFlask, FaInfoCircle, FaArchive
} from "react-icons/fa";
...
export default function AdminMidExamDashboard() {
  ...
  const getSubjectStats = (subId: string, subType: string) => {
    ...
    previewData.rows.forEach((row: any) => {
      ...
      if (previewType === "MID_I") {
        val = marksObj.mid1;
        isAbsent = marksObj.isMid1Absent || val === null || val === undefined;
      } else if (previewType === "MID_II") {
        val = marksObj.mid2;
        isAbsent = marksObj.isMid2Absent || val === null || val === undefined;
    ...
    try {
      if (selectedAnalysisExamType === "COMPARISON") {
        const [mid1Res, mid2Res, finalRes] = await Promise.all([
          fetch(`/api/mid-exam/analysis?academicYearId=${selectedAY}&departmentId=${selectedDept}&year=${selectedYear}&semester=${selectedSem}&sectionId=${selectedSection}&examType=MID_I`),
          fetch(`/api/mid-exam/analysis?academicYearId=${selectedAY}&departmentId=${selectedDept}&year=${selectedYear}&semester=${selectedSem}&sectionId=${selectedSection}&examType=MID_II`),
          fetch(`/api/mid-exam/analysis?academicYearId=${selectedAY}&departmentId=${selectedDept}&year=${selectedYear}&semester=${selectedSem}&sectionId=${selectedSection}&examType=FINAL`),
        ]);
  ...
  const generateAnalysisPDF = async (cachedData: any) => {
    ...
    try {
      ...
      const examLabel = metadata.examType === "MID_I" 
        ? "MID - I (30 Marks)" 
        : metadata.examType === "MID_II" 
          ? "MID - II (30 Marks)" 
          : "FINAL INTERNAL (30/50 Marks)";
      doc.text(`Exam: ${examLabel}`, 140, 50);
      ...
      if (theorySubjects.length > 0) {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(9);
        doc.text("Theory Subjects:", margin, currentY);
        currentY += 3.5;

        const theoryHeaders = [["S.No", "Code", "Subject Name", "Abs", "Avg", "Gap", "Diff %"]];
        ...
        autoTable(doc, {
          ...
          styles: { fontSize: 8.5, halign: "center", cellPadding: 2, lineColor: [80, 80, 80], lineWidth: 0.3 },
          columnStyles: {
            2: { halign: "left" }
          },
          headStyles: { fillColor: [241, 245, 249], textColor: [15, 23, 42], fontStyle: "bold", lineWidth: 0.4, lineColor: [60, 60, 60] }
        });
      ...
      if (labSubjects.length > 0) {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(9);
        doc.text("Lab / Practical Subjects:", margin, currentY);
        currentY += 3.5;

        const labHeaders = [["S.No", "Code", "Subject Name", "Abs", "Avg", "Gap", "Diff %"]];
      ...
      autoTable(doc, {
        ...
        startY: nextY + 3,
        margin: { left: margin, right: margin },
        styles: { fontSize: 8.5, halign: "center", cellPadding: 2.5, lineColor: [80, 80, 80], lineWidth: 0.3 },
        columnStyles: {
  ...
  const buildClassReportPDFBuffer = (reportType: "MID_I" | "MID_II" | "ASSIGNMENT" | "FINAL", data: any, logoBase64: string | null) => {
    ...
    const buildClassStatsFooter = () => {
      ...
      const theorySubjectsForStats = subjects.filter((s: any) => s.type?.toUpperCase() !== "LAB");
      const labSubjectsForStats = subjects.filter((s: any) => s.type?.toUpperCase() === "LAB");
      ...
      for (const sub of subjects) {
        avgRow.push(getSubjectAvg(sub.id, extractor));
      }
      if (reportType === "FINAL") avgRow.push("");
      footerRows.push(avgRow);

      const topLabel = reportType === "MID_I" || reportType === "MID_II" ? "Top (≥18 theory / ≥30 lab)" : "Top (≥18 theory / ≥30 lab)";
      const topRow: any[] = ["", "", topLabel];
      for (const sub of subjects) {
        const isLab = sub.type?.toUpperCase() === "LAB";
        const p = getSubjectPerformers(sub.id, extractor, isLab);
        topRow.push(p.top);
      }
      if (reportType === "FINAL") topRow.push("");
      footerRows.push(topRow);

      const midLabel = "Middle (12-17 theory / 20-29 lab)";
      const midRow: any[] = ["", "", midLabel];
      for (const sub of subjects) {
        const isLab = sub.type?.toUpperCase() === "LAB";
        const p = getSubjectPerformers(sub.id, extractor, isLab);
        midRow.push(p.middle);
      }
      if (reportType === "FINAL") midRow.push("");
      footerRows.push(midRow);

      const lowLabel = "Low (<12 theory / <20 lab)";
      const lowRow: any[] = ["", "", lowLabel];
      for (const sub of subjects) {
        const isLab = sub.type?.toUpperCase() === "LAB";
        const p = getSubjectPerformers(sub.id, extractor, isLab);
        lowRow.push(p.low);
      }
      if (reportType === "FINAL") lowRow.push("");
      ...
      for (const sub of subjects) {
        const isLab = sub.type?.toUpperCase() === "LAB";
        const p = getSubjectPerformers(sub.id, extractor, isLab);
        absentRow.push(p.absent);
      }
      if (reportType === "FINAL") absentRow.push("");
  ...
  const handleBulkDownload = async () => {
    if (bulkDownloadScope === "CURRENT" && !selectedSection) {
      showToast("Select a Section for bulk download", "error");
      return;
    }
    ...
    try {
      ...
      const fetchPromises = targetSections.map(async (secId) => {
        const res = await fetch(`/api/mid-exam/reports/memo?academicYearId=${selectedAY}&departmentId=${selectedDept}&year=${selectedYear}&semester=${selectedSem}&sectionId=${secId}`);
        if (!res.ok) {
      ...
      const scopeName = bulkDownloadScope === "CURRENT"
        ? `Section_${sections.find(s => s.id === selectedSection)?.name || "Current"}`
        : "All_Sections";
      link.download = `Bulk_Reports_${selectedDept}_Sem${selectedSem}_${scopeName}_${Date.now()}.zip`;
  ...
  const generateSubjectReportPDF = async (cachedData?: any) => {
    ...
    try {
      ...
      // Build Headers
      const isLabPDF = targetSubject?.type?.toUpperCase() === "LAB";
      const headers = [
        [
          "S.No", 
          "Roll Number", 
      ...
      const getColStatsPDF = (
        ...
        rows.forEach((r: any) => {
          const marksObj = r.subjects[selectedReportSubjectId] || {};
          const isAbsent = isAbsentExtractor(marksObj);
          const val = extractor(marksObj);
            ...
            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              ...
              {previewData && previewType && (
                <div id="report-preview-pane" className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100 space-y-6">
                            ...
                            <>
                              ...
                              <thead>
                                <tr className="bg-slate-50 border border-black text-slate-600 font-bold uppercase">
                                  ...
                                  {previewType === "FINAL" && (
                                    <th className="px-4 py-3 text-center border border-black min-w-[75px] bg-slate-100 font-bold">
                                      TOTAL
                                    </th>
                                  )}
                                </tr>
                                  ...
                                  <tr key={row.rollNumber} className="hover:bg-slate-50/50">
                                    ...
                                    {filteredSubjects.map((sub: any) => {
                                      ...
                                      if (previewType === "MID_I") {
                                        ...
                                        val = marksObj.mid2;
                                        displayVal = marksObj.isMid2Absent ? "AB" : (val !== null ? Math.round(val).toString() : "AB");
                                      } else if (previewType === "ASSIGNMENT") {
                                        val = marksObj.assignment;
                              ...
                              {previewType !== "FINAL" && previewType !== "ASSIGNMENT" && (
                          ...
                          <>
                            ...
                            <tfoot>
                              <tr className="bg-slate-50 border-t border-b border-slate-200 font-bold text-slate-800">
                                ...
                                <td className="px-4 py-3 text-center text-sm font-bold border border-black text-slate-700"></td>
                              ...
                              {/* Above 60% Row */}
                              <tr className="bg-white text-slate-700 font-bold border-b border-black">
                                <td className="px-4 py-2 text-center border border-black"></td>
                                <td className="px-4 py-2 text-center border border-black"></td>
                                <td className="px-4 py-2 text-right border border-black text-[11px] font-medium text-slate-600">
                                  Total No. of Students Above 60% {isLab ? "(30 and above)" : "(18 and above)"}
                                </td>
                                {showAttendance && <td className="px-4 py-2 text-center border border-black"></td>}
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid1.countAbove}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid2.countAbove}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsFinal.countAbove}</td>
                              </tr>
                              ...
                              <tr className="bg-white text-slate-700 font-bold border-b border-black">
                                ...
                                <td className="px-4 py-2 text-center border border-black"></td>
                                <td className="px-4 py-2 text-right border border-black text-[11px] font-medium text-slate-600">
                                  Total No. of Students Between 60% to 40% {isLab ? "(29 - 20 Marks)" : "(17 - 12 Marks)"}
                                </td>
                                {showAttendance && <td className="px-4 py-2 text-center border border-black"></td>}
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid1.countBetween}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid2.countBetween}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsFinal.countBetween}</td>
                              </tr>
                              ...
                              <tr className="bg-white text-slate-700 font-bold border-b border-black">
                                ...
                                <td className="px-4 py-2 text-center border border-black"></td>
                                <td className="px-4 py-2 text-right border border-black text-[11px] font-medium text-slate-600">
                                  Total No. of Students Below 40% {isLab ? "(19 - 1 Marks)" : "(11 - 1 Marks)"}
                                </td>
                                {showAttendance && <td className="px-4 py-2 text-center border border-black"></td>}
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid1.countBelow}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid2.countBelow}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsFinal.countBelow}</td>
                              </tr>
                              ...
                              <tr className="bg-white text-slate-700 font-bold border-b border-black">
                                ...
                                {showAttendance && <td className="px-4 py-2 text-center border border-black"></td>}
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid1.countZero}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid2.countZero}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsFinal.countZero}</td>
                              </tr>
                              ...
                              <tr className="bg-white text-slate-700 font-bold border-b border-black">
                                ...
                                {showAttendance && <td className="px-4 py-2 text-center border border-black"></td>}
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid1.countAbsent}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsMid2.countAbsent}</td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black"></td>
                                <td className="px-4 py-2 text-center text-xs font-semibold border border-black bg-slate-50/30">{statsFinal.countAbsent}</td>
                              </tr>
                            </tfoot>
                          </>
                        );
                  ...
                  {previewType !== "SUBJECT" && previewType !== "FINAL" && previewType !== "ASSIGNMENT" && (() => {
                                 ...
                                 {comparativeLabSubjects.map((sub: any, idx: number) => (
                                   <tr key={sub.subjectCode} className="hover:bg-slate-50/50">
                                     ...
                                     <td className="px-1 py-2 border-r border-slate-200">
                                       <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${sub.mid1.insight === "Good" ? "bg-emerald-100 text-emerald-700" : sub.mid1.insight === "Moderate" ? "bg-amber-100 text-amber-700" : sub.mid1.insight === "N/A" || !sub.mid1.insight ? "text-slate-400" : "bg-red-100 text-red-700"}`}>{sub.mid1.insight || "N/A"}</span>
                                     </td>
                                     <td className="px-1 py-2 border-r border-slate-200">
                                       <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${sub.mid2.insight === "Good" ? "bg-emerald-100 text-emerald-700" : sub.mid2.insight === "Moderate" ? "bg-amber-100 text-amber-700" : sub.mid2.insight === "N/A" || !sub.mid2.insight ? "text-slate-400" : "bg-red-100 text-red-700"}`}>{sub.mid2.insight || "N/A"}</span>
                                     </td>
                                     <td className="px-1 py-2 border-r border-slate-200">
                                       <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${sub.final.insight === "Good" ? "bg-emerald-100 text-emerald-700" : sub.final.insight === "Moderate" ? "bg-amber-100 text-amber-700" : sub.final.insight === "N/A" || !sub.final.insight ? "text-slate-400" : "bg-red-100 text-red-700"}`}>{sub.final.insight || "N/A"}</span>
                                     </td>

                                     <td className="px-1 py-2 border-r border-slate-200">
                                       <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${sub.mid1.remarks === "Easy" ? "bg-emerald-100 text-emerald-700" : sub.mid1.remarks === "Moderate" ? "bg-amber-100 text-amber-700" : sub.mid1.remarks === "N/A" || !sub.mid1.remarks ? "text-slate-400" : "bg-red-100 text-red-700"}`}>{sub.mid1.remarks || "N/A"}</span>
                                     </td>
                                     <td className="px-1 py-2 border-r border-slate-200">
                                       <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${sub.mid2.remarks === "Easy" ? "bg-emerald-100 text-emerald-700" : sub.mid2.remarks === "Moderate" ? "bg-amber-100 text-amber-700" : sub.mid2.remarks === "N/A" || !sub.mid2.remarks ? "text-slate-400" : "bg-red-100 text-red-700"}`}>{sub.mid2.remarks || "N/A"}</span>
                                     </td>
                                     <td className="px-1 py-2">
                                       <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${sub.final.remarks === "Easy" ? "bg-emerald-100 text-emerald-700" : sub.final.remarks === "Moderate" ? "bg-amber-100 text-amber-700" : sub.final.remarks === "N/A" || !sub.final.remarks ? "text-slate-400" : "bg-red-100 text-red-700"}`}>{sub.final.remarks || "N/A"}</span>
                                     </td>
                                   </tr>
                                 ))}
                               </tbody>
